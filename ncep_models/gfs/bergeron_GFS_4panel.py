...
data = xr.open_dataset('http://nomads.ncep.noaa.gov:9090/dods/{}/{}{}/{}_{}z'.format(model,
                                                                                     model[0:3],
                                                                                     date[:-2],
                                                                                     model,
                                                                                     run))
...
for fh in range(ntimes):
    ...
    wspd_300 = mpcalc.get_wind_speed(uwnd_300, vwnd_300)
    ...
    if fh == 0:
        ...
        cf = ax1.contourf(tlon,tlat,pcpin,clevprecip,cmap=cmap)
...
#               transform=datacrs)
    ...
    plt.clabel(cs,fontsize=10,inline=True,fmt='%d',fontwieght='heavy',rightside_up=True)
    plt.clabel(cs2,fontsize=9,inline=True,fmt='%d',fontwieght='heavy',rightside_up=True)
