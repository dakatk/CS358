...
clev500 = np.arange(4200,6000,60)
...
states_provinces = cfeature.NaturalEarthFeature(
        ...
        scale='50m',
        facecolor='none')
tlatlons = crs.transform_points(ccrs.Miller(),clats,clons)
clats = tlatlons[:,:,0]
clons = tlatlons[:,:,1]

LL = crs.transform_point(-125.,22.,ccrs.Miller())
UR = crs.transform_point(-55.,52.,ccrs.Miller())

print LL[0],LL[1]
print UR[0],UR[1]

print clats[0,0],clons[0,0]
print clats[99,179],clons[99,179]
...
# Upper-left panel MSLP, 1000-500 hPa Thickness, Precip (in)
  ax1 = plot_background(221,crs)
#  ax1 = plt.subplot(221,projection=crs)
#  ax1.set_extent([LL[1],LL[0],UR[1],UR[0]],crs)
...
# set_extent([west long, east long, south lat, north lat])
  cmap = metcm.s3pcpn_l
  if fh % 2 != 0:
     #cf = ax1.contourf(clats,clons,pcpin[fh,:,:],clevprecip,cmap=cmap,latlon=True,transform=ccrs.Miller())
     cf = ax1.contourf(clats,clons,pcpin[fh,:,:],clevprecip,cmap=cmap,latlon=True)
  else:
     cf = ax1.contourf(clats,clons,pcpin[fh,:,:]-pcpin[fh-1,:,:],clevprecip,cmap=cmap,latlon=True)
  cbar = plt.colorbar(cf,orientation='horizontal',extend='both',aspect=65,shrink=0.913,pad=0,extendrect='True')
  cs2 = ax1.contour(clats,clons,Z_500[fh,:,:]-Z_1000[fh,:,:],clev500,colors='r',linewidths=1.5,linestyles='dashed',latlon=True)
  cs  = ax1.contour(clats,clons,pmsl[fh,:,:],clevpmsl,colors='k',linewidths=1.5,latlon=True)
...
#  ax2 = plt.subplot(222,projection=crs)
  ax2 = plot_background(222,crs)
#  ax2.set_extent([LL[1],LL[0],UR[1],UR[0]],crs)
#  plt.subplots_adjust(bottom=0, left=.01, right=.99, top=.99, hspace=.5,wspace = 0.1)
#  ax2.coastlines('50m',edgecolor='black',linewidth=0.5)
#  ax2.add_feature(cfeature.BORDERS, linestyle='-')
#  ax2.add_feature(states_provinces,edgecolor='black',linewidth=0.5)
#  set_extent([west long, east long, south lat, north lat])
#  ax2.set_extent([-135,-55, 20, 50])
  cmap = plt.cm.jet
  norm = MidpointNormalize(midpoint=10)
  cf = ax2.contourf(clats,clons,tmpc850[fh,:,:],clevtmpc850,cmap=cmap,norm=norm,extend='both',latlon=True)
  cbar = plt.colorbar(cf,orientation='horizontal',extend='both',aspect=65,shrink=0.913,pad=0,extendrect='True')
  cs = ax2.contour(clats,clons,Z_850[fh,:,:],clev850,colors='k',linewidths=1.5,latlon=True)
  plt.clabel(cs, fontsize=8, inline=1, inline_spacing=10, fmt='%i', rightside_up=True)
...
#  ax3 = plt.subplot(223,projection=crs)
  ax3 = plot_background(223,crs)
#  ax3.set_extent([LL[1],LL[0],UR[1],UR[0]],crs)
#  plt.subplots_adjust(bottom=0, left=.01, right=.99, top=.99, hspace=.5, wspace = 0.1)
#  ax3.coastlines('50m',edgecolor='black',linewidth=0.5)
#  ax3.add_feature(cfeature.BORDERS, linestyle='-')
#  ax3.add_feature(states_provinces,edgecolor='black',linewidth=0.5)
#  cmap = plt.cm.get_cmap("YlOrBr")
  cf = ax3.contourf(clats,clons,avor500[fh,:,:],clevavor500,colors=colorsavor500,extend='both',latlon=True)
  cbar = plt.colorbar(cf,orientation='horizontal',extend='both',aspect=65,shrink=0.913,pad=0,extendrect='True')
  cs = ax3.contour(clats,clons,Z_500[fh,:,:],clev500,colors='k',linewidths=1.5,latlon=True)
...
#  ax4 = plt.subplot(224,projection=crs)
  ax4 = plot_background(224,crs)
#  ax4.set_extent([LL[1],LL[0],UR[1],UR[0]],crs)
#  plt.subplots_adjust(bottom=0, left=.01, right=.99, top=.99, hspace=.5, wspace = 0.1)
#  ax4.coastlines('50m',edgecolor='black',linewidth=0.5)
#  ax4.add_feature(cfeature.BORDERS, linestyle='-')
#  ax4.add_feature(states_provinces,edgecolor='black',linewidth=0.5)
  cmap = plt.cm.get_cmap("BuPu")
  cf = ax4.contourf(clats,clons,wspd300[fh,:,:],clevsped300,cmap=cmap,extend='max',latlon=True)
  cbar = plt.colorbar(cf,orientation='horizontal',extend='max',aspect=65,shrink=0.913,pad=0,extendrect='True')
  cs = ax4.contour(clats,clons,Z_300[fh,:,:],clev300,colors='k',linewidths=1.5,latlon=True)
  plt.clabel(cs, fontsize=8, inline=1, inline_spacing=10, fmt='%i', rightside_up=True)
...
# Save the figure to a unique name every time.
  plt.savefig('/home/kgoebber/http/ncep_models/gfs/gfs_four_panel_f%02d.png' % (fh*time_step),dpi=150)
